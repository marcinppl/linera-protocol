...
#[cfg(feature = "fs")]
impl<K> WalletState<persistent::File<Wallet<K>>> {
...
#[cfg(with_indexed_db)]
impl<K> WalletState<persistent::IndexedDb<Wallet<K>>> {
    pub async fn create_from_indexed_db(key: &str, wallet: Wallet<K>) -> Result<Self, Error>
    where
        K: Serialize + DeserializeOwned,
    {
        Ok(Self::new(
